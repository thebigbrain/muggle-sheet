...
const installModule = mod => {
  ...
  for (let k in ns.actions) {
    modActions[k] = (...args) => {
      ns.actions[k]({ state, actions, effects }, ...args);
  ...
  if (ns.onInitialize) ns.onInitialize({ state, actions, effects }, store);
};

function renderUi(ui) {
  const mod = installPlugin(ui.name, ui.source);
  if (!mod) {
    return null;
  }
  ...
  if (!INSTALLED_MODULE_CACHE.has(ui.name)) {
    installModule(mod);
    INSTALLED_MODULE_CACHE.set(ui.name, mod);
  }

  return props => {
    return mod.component ? <mod.component {...props} /> : null;
  };
}
